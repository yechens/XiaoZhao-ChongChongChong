...
解题思路：
定义二维 dp，dp[i][j] 表示：word1 到 i 位置转换成 word2 到 j 位置需要最少的步数
...
class Solution:
    def minDistance(self, word1: str, word2: str) -> int:
        ...
        dp = [[0] * (n + 1) for _ in range(m + 1)] # n+1/m+1 是为了更方便处理边界情况
        # 初始化第一行和第一列
        for i in range(1, m+1):
            dp[i][0] = i # 相当于 word2 = “” 的情况
        for j in range(1, n+1):
            dp[0][j] = j # 相当于 word1 = “” 的情况
        ...
        for i in range(1, m+1):
            for j in range(1, n+1):
                if word1[i-1] == word2[j-1]:
                    ...
                    dp[i][j] = min(dp[i-1][j-1], dp[i][j-1], dp[i-1][j]) + 1 # 3个操作取最小！
